    ...
    return (
        <MessageListStyles className="w-100 flex column gap-2">
            <div className="chatList w-90 auto flex column gap-2">
                {
                    message?.map((messages?: { sender: any; createdAt: moment.MomentInput; body: any }) => {
                        const usermessage = messages?.sender.id === userInfo?.id
                        const createdAt = moment(messages?.createdAt).format('MMMM Do YYYY, h:mm a')
                        return <div className="">
                            <div className="chatCard flex w-100 column">
                                {
                                    usermessage ? <div className="flex w-100 column gap-1">
                                        <div className="revieverWrapper flex item-center gap-1">
                                        ...
                                        : <div style={{justifyContent:"flex-start"}} className="flex w-100 column gap-1">
